...
/* 
 * byteNot - bit-inversion to byte n from word x9-  
...
int mul2OK(int x) {
    ...
    i=(~i);  
    j=x&(i<<31);  //得到x的最高位
    k=(x&(i<<30))<<1;  //得到次高位并移到最高位
    return (((~j)>>31)^((~k)>>31))+1;  //因为没有!，所以要复杂一些
}
/*
...
unsigned float_abs(unsigned uf) {
    ...
    unsigned frac = uf&0x7fffff;  //使前8位都为0
    if (exp==0xff){
        if(frac!=0){
            return uf;
        }
